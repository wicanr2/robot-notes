...
| **做叉車 / 搬運車的控制器** | [回授控制:PID、LQR 與 MPPI](docs/90-foundations/feedback-control-pid-lqr.md) — 哪一層該用 PID、哪一層該用 LQR,三條控制律都從第一性原理推起,後半整章落到叉車 |
...
- [回授控制:PID、LQR 與 MPPI](docs/90-foundations/feedback-control-pid-lqr.md) — 內模原理為什麼逼出 I 項、代價為什麼是 xᵀQx+uᵀRu、u=−Kx 與 Riccati 方程怎麼推出來、LQR 保證什麼又在哪裡失效;取樣式 MPC(MPPI)如何用 Gibbs 變分原理繞過「代價必須是二次型」,以及 Nav2 的實際參數;後半落到叉車:舵輪運動學、四層迴路分工、後輪轉向的右半平面零點、載重的增益排程
