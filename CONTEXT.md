...
- Capture point — `ξ = x + ẋ√(z₀/g)`,踩上去能讓重心速度漸近收斂到零的落腳點。定高倒單擺的通解是 `A·e^{ωt} + B·e^{−ωt}`,而 `A = ½[(x−p) + ẋ/ω]`——**所以 ξ 不是湊出來的,它就是「現在帶了多少發散成分」**。踩在 ξ 上等於把 `A` 設成零。**機制是消掉發散模態,不是「動能被抬升重心的位能吃光」**(重心高度固定,沒有位能可吃)。ZMP 的式子裡沒有速度,capture point 的式子裡速度是主角。注意 `ξ = p` 是**不穩定**平衡,要逐週期重算。
