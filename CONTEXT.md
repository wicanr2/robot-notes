...
- 離散拉普拉斯算子(smoother)— `y_{i+1} + y_{i−1} − 2y_i`,把每個點往相鄰兩點的中點拉。Simple Smoother 的更新式就是「錨定項 + 這一項」,而它正是[離散平滑器目標函數](docs/10-core/30-navigation/path-smoothing-and-trajectory.md)的梯度下降。**它不看 costmap、也會破壞運動學可行性**,官方明訂只能配 2D 規劃器。
