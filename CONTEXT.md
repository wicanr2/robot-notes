...
- 接觸集合(contact set)— 當下哪幾隻腳踩在地上。四足有 2⁴ = 16 種,每一種對應一組不同的動力學方程式。
...
- Capture point — 下一步踩上去能讓動能剛好被抬升重心吃光的落腳點。把穩定變成「還剩幾步可以救」的餘裕。ZMP 管腳與地的介面,capture point 管整台機器的能量。
- Duty factor — 一隻腳在一個步態週期裡觸地的時間比例。**> 0.5 才可能靜態穩定,< 0.5 必有飛行相**——這是幾何必然不是經驗值。
