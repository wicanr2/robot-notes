...
- C 項(離心 + Coriolis)— 「慣量隨構型變」在方程裡的倒影:M 是常數矩陣則 C ≡ 0。速度平方項是離心、交叉項是 Coriolis;合起來**不做功**(Ṁ−2C 反對稱)。
...
- 重力補償 — `τ = Kvė + Kpe + N(θ)`:PD 之外只前饋重力。定點調節全域穩定,靠的是 C 不做功這條性質。
- RNEA(recursive Newton-Euler algorithm)— O(n) 逆動力學:向外一趟累積速度加速度、向內一趟回收力與力矩。與 Lagrange 閉式是同一組物理的兩種計算安排。
- 慣性參數線性性 — 方程可寫成 `Y(θ,θ̇,θ̈)π`,對慣性參數向量 π 是線性的;參數辨識因此是線性回歸,adaptive control 靠它在線修參數。
