...
## R17.1 修正的兩個錯誤(都不是筆誤,是推論本身錯了)

1. **把來源的「統計上偏向」升級成「幾何上的必然」。** 研究資料寫的是四足步態「統計上偏向」以 0.5 分界,我改寫時寫成「這是幾何必然不是經驗值」——加強的那一部分沒有任何證據。正確的通則是 `1/n`,而 n 由相位怎麼排決定。**判斷法:寫「必然 / 一定 / 幾何上」之前,要能當場把證明寫出來。**
2. **解釋機制比寫對公式更容易錯,也更難被抓到。** capture point 的公式沒寫錯,但配上的物理解釋(動能換位能)跟模型假設(定高)直接矛盾,也跟我自己引的兩份來源矛盾。「能量守恆」這類解釋最危險,因為讀起來太順,沒人會停下來檢查。

> 兩個錯都是審查抓的,不是我自己複查抓的。學生的質疑(「照文中 walk 那張圖的排法算,門檻是 0.25 不是 0.5」)先到,專家獨立確認並補上文獻反例。**審查角色分開派仍然有效——學生問「憑什麼」,專家答「對照哪一篇」。**
