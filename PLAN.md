...
3. **既有的筆記就是新篇的骨架,前提是先去找。** Simple Smoother 的原始碼更新式一看就眼熟——驗算之後確認它**正是** `path-smoothing-and-trajectory.md §6` 那個離散平滑器目標函數的梯度下降(對 `y_i` 取偏導、把常數吸收進權重就得到那一行)。這個連結讓新篇不必重推,也讓舊篇多了一個具體落點。
