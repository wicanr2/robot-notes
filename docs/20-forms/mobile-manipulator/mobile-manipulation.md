...
回到那條速度映射 `J q̇ = ẋ`。當 `J` 的行數(可控關節)多於列數(任務維度)時,這是一個**欠定**方程組——未知數比方程式多,解有無限多個。
...
**MoveIt Servo** 是另一條路徑:不先規劃再執行,而是直接串流速度指令,用於遙操作或視覺伺服。值得注意的是它**內建奇異點偵測,靠近時自動降速**——[手臂運動學](arm-kinematics.md#33-奇異點少一個方向)推導出「`J⁻¹` 會發散所以必須提前限速」這件事,在成熟的工具裡是預設行為。
