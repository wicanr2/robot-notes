...
差別在哪裡:ZMP 是一個**瞬時**條件,只看「此刻壓力中心在不在接觸面內」。機器人可以整段時間都滿足它,卻一路把重心往前加速——每一瞬間腳都平貼在地、ZMP 完全合法,但速度越來越大,直到重心衝出腳掌能反應的範圍。**合法的是每一格,不合法的是整段軌跡。** 而且飛行相沒有接觸面,ZMP 依然無從定義。
...
**capture point** 就是那個位置。它的推導只需要一個模型與一行代數:

把機器人簡化成**重心維持在固定高度 z₀** 的倒單擺(linear inverted pendulum),支點在地面的 `p`,水平方向的動力學是

```
ẍ = ω²(x − p)   ,   ω = √(g / z₀)
```

這個系統本身是發散的。但如果定義 `ξ = x + ẋ/ω`,代進去會得到

```
ξ̇ = ẋ + ẍ/ω = ω(x + ẋ/ω − p) = ω(ξ − p)
```

`ξ` 只受一個東西影響:**它自己離支點多遠**。把支點踩在 `ξ` 上,`ξ̇ = 0`,發散項就此消失,重心軌跡變成 `x(t) = p + (x₀ − p)e^{−ωt}`——指數收斂到落腳點,速度趨近零。

所以 `ξ = x + ẋ√(z₀/g)` 就是 capture point:**重心的水平位置,加上速度乘一個由腿長決定的時間常數。**

> **不要把它想成「動能被抬高重心的位能吃掉」。** 這個模型裡重心高度全程不變,沒有位能可以吃。真正發生的事是**主動把發散模態消掉**——踩對地方,不穩定的那一項係數歸零。這也解釋了為什麼收斂是漸近的(指數趨近,不是在某個時刻「剛好用完」),以及為什麼 capture point 只跟重心的位置、速度和 ω 有關,跟「腿還能伸多長」無關。
