...
| **connection**(連線) | broker → 主控 | 用 MQTT last-will 偵測車輛非預期斷線 |
| **factsheet**(規格表) | 車 → 主控 | 車輛能力宣告:尺寸、載重、支援的 action |

> QoS 設計也呼應第一性原理:order/state/visualization 多用 QoS 0(best effort,夠快);**connection 用 QoS 1**(at least once),因為「斷線」這個安全訊號絕不能漏。
...
|---|---|
| `NONE` | 可與行駛並行執行 |
| `SOFT` | 必須停下車序列執行,但可被取消 |
| `HARD` | 必須停下車序列執行,且不可取消 |
