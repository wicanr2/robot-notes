...
- 作業系統的 **banker's algorithm(銀行家演算法)** 給了一條中間路線——它正好是「策略二要把序列化做對」的理論版本。為什麼?下面 §2.1 用這題的例子走一次。

> 一句話:策略一 = 悲觀、保守、簡單;策略二 = 樂觀、高吞吐,但正確性全押在「序列化與釋放條件」做不做得紮實。

### 2.1 為什麼 banker's algorithm 正好是這題的「中間路線」

**先講 banker's 本身**(Dijkstra 提的死結**避免**演算法)。把作業系統想成一個銀行:銀行有固定的資本,幾個客戶各自會借錢、用完再還。規則是——每個客戶**一開始就要宣告「我最多會借到多少」**;之後每次有人來要錢,銀行不是有錢就借,而是先做一次沙盤推演:「**如果借了,是否還存在一種還款順序,讓所有客戶都能依序借滿、用完、還清?**」存在這種順序就叫**安全狀態**,那個順序叫**安全序列**;只要還在安全狀態就放款,否則就算金庫有錢也先讓你等。只要每一步都維持安全狀態,就**保證不會死結**(不會所有客戶都借一半、卡住誰也還不了)。
...
四筆都能依序收尾 → **安全序列存在 → banker's 准許全部排入**。這就是策略二理想中的「自動交錯流水」,而 banker's 是它的**形式化證明**:不是放行後靠 runtime gating 祈禱,而是**放行前就證明整批有完成順序**。

反過來,假設又有人補一筆,但場上沒有任何任務會把它依賴的儲位空出來(例如該取走 B 的那台車自己又卡在等別的資源),banker's 一跑安全檢查發現「湊不出任何完成順序」,就把這筆擋在門外。**注意它擋的理由是「整批湊不出安全順序」,不是「B 出現了第二次」**——這正是它比策略一聰明的地方。

**這就是為什麼它叫「中間路線」**。死結有三種對付法,正好把三者擺定位:

- **預防(prevention)**:從結構上讓死結四條件之一不可能成立。**策略一**就是預防——直接讓「同一格不被兩筆持有」,手段最粗,連很多其實安全的組合也一起擋掉。
- **避免(avoidance)**:允許請求進來,但每次放行前先算「放了還安全嗎」,不安全才讓它等。**banker's 就是這一類**,也正是 naive 策略二缺的那一塊。它比策略一**少擋**(只擋真正不安全的),又比 naive 策略二**安全**(放行前就有完成順序的保證)。
- **偵測 + 復原(detection + recovery)**:先讓它跑,真的死結了再偵測出來,然後 abort / 重排某些任務解套。

**誠實講它的限制**(所以實務很少直接照搬教科書版):banker's 要求**每筆任務事先宣告最大資源需求**,而且每次放行都要跑一次安全檢查(單次成本約 `O(n²m)`,`n` 任務數、`m` 資源種類)。車隊的資源還包含**動態的路段、車輛位置**,數量大又一直變,要全域枚舉最大需求並不現實。所以業界走的是它的**後代**——[§3.4 的「帶時間窗動態預定」](#34-學界目的資源衝突有成熟解法且偏向帶時間窗的動態預定):把「安全序列」這個想法**加上時間維度**(在某段時間窗預定 B),保留 deadlock-free 的精神,又不必枚舉全域最大需求。一句話:**banker's 是策略二的理論天花板,實務則用它的時間窗版本來逼近。**
...
這帶出一個現場常見的張力:**若場域有強勢上位系統(WMS/WES),它本來就會一次塞一串任務下來**。這時車隊層若採策略一(直接拒收第二筆放 B),等於把上位的職責拉回車隊層、製造雙重事實來源,且會讓「習慣一次塞滿」的上位系統大量任務被擋下。策略二(車隊只負責序列化執行)更貼近這種主流分工。([WCS/WES/WMS 分工](https://kpisolutions.com/resources/wcs-vs-wes-vs-wms-the-software-behind-warehouse-operations/))
...
> **分工**:四層架構中 *"the **host system, usually a WMS or WES, releasing transport orders** … Below that sits the **fleet manager, which performs task allocation, route reservation, traffic control**, battery management, and recovery logic."*;實務建議 *"let the WMS or WES **issue transport missions at a business level**, let the **fleet manager sequence and route them**, and let station PLCs own all local motion."*
>
> 對照本題:**上位下「商業層」任務、車隊層負責 sequence(序列化)與路由**——這正是策略二的分工;而 route reservation(路徑/資源預定)被明確劃在車隊層,不是上位。

> **重複預定的直接後果**:它在「重啟/復原」一節特別警告——*"the architecture should make clear … whether the **WMS should hold or resend the transport order. Ambiguity here creates duplicate missions and inventory mismatches.**"*
>
> 也就是說,「**該由誰、在什麼條件下 hold 或重送任務**」沒定義清楚,現場就會冒出**重複任務與庫存錯亂**——這正是本篇「目的點重複預定」要回答的問題:hold(策略一/序列化卡住)還是 resend(再派),以及釋放/觸發條件怎麼定。
...
- **釋放條件要看物理佔用,不能只看任務狀態**:這是最容易錯的一點。「B 何時算空出來」要定義成「貨真的被取走 / 車真的離開 B」,而不是「上一筆任務的狀態變成 done」——否則會出現「任務說完成了、車還停在 B 上,下一台就撞上去」(正是 Open-RMF mutex 那個已知坑)。倉儲現場也有獨立佐證:[那篇 AGV 架構文章](https://www.smartloadinghub.com/insights/agv-amr/designing-agv-fleet-architecture-reliable-warehouse/)說 *"If the state model is weak, the **fleet will dispatch the next move too early** and create a cascade of stranded loads."*——狀態模型(load present / transfer complete / station occupied)不準,車隊就會**太早觸發下一步**,造成一連串卡死的貨。釋放條件要綁在這些**物理訊號**上,不是任務旗標。
