...
前面那些延遲數字不是抽象的服務等級,它們會**直接進到交管的公式裡**。

[路網與交通管制](roadnet-and-traffic-control.md)那篇推導的前導線距離是:

```
D_lead = L_head + v·(2τ + t_p) + v²/(2a) + b
```

其中 `τ` 就是**單程通訊延遲**,`t_p` 是調度端的處理時間。`2τ` 出現在這裡的理由很實在:車回報位置要花 `τ`、指令傳回去又要花 `τ`,這段期間車還在往前開,而調度器手上那筆位置**已經過期了**。
...
**所以連線方式的選擇不只是「連不連得上」,它會改變路網能排多密。** 同一段公式還告訴你另一件事:延遲的**抖動**比延遲本身更麻煩——`τ` 若在 30 到 300 ms 之間跳,`D_lead` 就得按最壞情況取,整條路的通行量被最差的那一刻決定。
