...
要判斷目標在車的前方還後方,得同時知道三件事:

1. 車**當前的位置**(pose);
2. 車頭**朝哪**(舵輪端的方向 θ);
3. 目標點的座標。

有了這三個,用一條投影就能定號。設車 pose 為 `(xᵣ, yᵣ, θᵣ)`,目標為 `(xₜ, yₜ)`,把「車到目標」的向量投影到車頭方向:
...
代案例:`forward ≈ (−9.527 −(−8.549))·cos 0 + (微小)·sin 0 ≈ −0.978 < 0` → 應倒車。系統卻給了 `+0.5`,方向剛好相反,大迴轉是這個符號錯誤的必然結果。

重點不在這條公式,而在**它需要的三個輸入,只有車端即時掌握**。派發層在規劃當下,不知道車最後會停在哪、車頭朝哪。私有系統把 `vel` 寫死在路網定義裡、由派發層下發,等於**讓不知道「目標在我前面還後面」的層,替車決定了往哪頭開**。這是責任錯置,不是運動學限制。
...
|---|---|---|---|
| 拓樸分配層 | 接回哪個 waypoint / 哪條 lane | `rmf_traffic` + fleet adapter 的 **lane merge** | 不碰。只給「去 W 點」 |
| 運動規劃層 | 從當前 pose 怎麼開過去(前進/倒車、轉彎半徑、軌跡) | **車端 Nav2**(controller / planner) | **這層的權責**,內建車輛運動學 |

RMF 給車的是**拓樸目標**(「去這個 waypoint」),它**不生成軌跡、不指定速度符號**。「現在該前進還倒車」是車端 Nav2 拿著即時 pose 自己算的。Open-RMF 還內建一個門檻參數(EasyFullControl 的 merge waypoint / lane 距離,參數名以實作為準),用來判定「車還在 lane 上,還是已經偏出去算 lost」,這正好是私有系統缺的那個「車在不在軌道上」的旗標。
...
任何舵輪 / Ackermann 車從任意朝向接目標,因為有最小轉彎半徑,一定會有一段迴轉。但這台 reach truck 的舵輪可轉到接近 ±90°,最小轉彎半徑很小,甚至能繞牙叉端兩輪做近似原地掉頭。所以它本來就**有能力**小半徑接回,不該被迫走大圈。

**(b) 案例這種「前進追後方點」的病態大迴轉 — 結構上不會。** 因為:

1. Nav2 是 free-space(costmap)規劃,目標在後方就直接規劃倒車或最短可行路徑,不被「沿用前段速度符號」綁死。
2. 速度符號由 controller 依運動學即時決定。Nav2 的 Smac Hybrid-A* 規劃器有 `motion_model_for_search`:選 Dubins 只准前進,選 **Reeds-Shepp 則允許倒車**,配合 `min_turning_radius` 會自動選「倒一小段」而不是「前進繞一大圈」。
...
- Open-RMF EasyFullControl 判定 on-lane / lost 的確切參數名與預設值(核對 `rmf_ros2` 原始碼)。
- Nav2 Smac Hybrid-A* 在窄空間倒車接駁的實測行為與調參(`reverse_penalty`、`change_penalty` 等對繞行的影響)。

## 8. 來源

- 拓樸 vs 運動規劃分層:[Open-RMF](open-rmf.md)、[rmf_traffic](https://github.com/open-rmf/rmf_traffic)。
- nonholonomic 規劃:[Nav2 Smac Planner](https://docs.nav2.org/configuration/packages/configuring-smac-planner.html)(Dubins / Reeds-Shepp、`min_turning_radius`)。
