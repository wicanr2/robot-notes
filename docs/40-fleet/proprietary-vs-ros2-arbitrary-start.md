...
RMF 給車的是**拓樸目標**(「去這個 waypoint」),它**不生成軌跡、不指定速度符號**。「現在該前進還倒車」是車端 Nav2 拿著即時 pose 自己算的。Open-RMF 的 fleet adapter(例如 EasyFullControl 這個現成模式)還有一組距離門檻參數,把車回報的 pose **吸附**到 nav graph 上最近的 waypoint / lane(參數名以實作為準);車偏出超過門檻,就視為離開路網。RMF 沒有一個現成叫「lost」的狀態,但這個吸附門檻剛好可以當「車在不在軌道上」的判斷依據,正是私有系統缺的那個旗標。
...
2. 倒車與否主要在 **planner 階段**決定。Nav2 的 Smac Hybrid-A* 規劃器有 `motion_model_for_search`:選 Dubins 只准前進,選 **Reeds-Shepp 則允許倒車**,配合 `min_turning_radius` 規劃出「倒一小段」而不是「前進繞一大圈」的路徑;controller(如 RPP / MPPI)再去追隨這條路徑、輸出帶符號的速度。
