...
$$ F_{\text{traction}}^{\max} = \mu \cdot N $$

3. 抓不抓得住:輪子要前進需要切向牽引力 $F_t$。$F_t \le \mu N$ → 抓住地、正常前進;$F_t > \mu N$ → **多出來的傳不出去,牽引力卡在 $\mu N$,輪子開始打滑空轉**(次方向同理用 `mu2`)。
4. `slip1` / `slip2`(ODE 的「力相依滑移」):就算還在摩擦上限內,接觸點仍容許一點滑移,滑移速度正比於切向力與 slip 係數:

$$ v_{\text{slip}} = \text{slip} \cdot F_t $$

slip 越大,同樣的力滑得越多。
5. 車身前進:把牽引力減掉阻力,套牛頓第二定律積分成速度與位移:

$$ a = \frac{F_{\text{traction}} - F_{\text{resist}}}{m},\qquad v=\int a\,dt $$

把 $\mu = 0.05$(很小)配上高 `slip`:$\mu N$ 一下就被超過、$v_{\text{slip}}$ 又大 → 輪子轉得快、車卻幾乎沒前進。但輪式 odom 是拿「輪子轉了多少」回推位移,於是它**以為車走了一大段**——漂移就是這個「輪子轉動 vs 實際位移」的落差。
