...
定子不動,固定在輪軸上:矽鋼片疊成的鐵芯,開槽,繞上三組線圈 U/V/W,三組線圈在圓周上每隔 120°(電氣角)分布;§11 的三顆霍爾開關也嵌在定子上;§12 功率橋的三條輸出線,接的就是定子的 U/V/W。轉子則是轉動的部分(= 輪框):一圈永久磁鐵,N/S 交替排列,§11.1 的「極對數」就是數它。下圖(§17.3)可以看到這個構造的完整剖面。
