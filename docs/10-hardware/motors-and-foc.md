...
定子不動,固定在輪軸上:矽鋼片疊成的鐵芯,開槽,繞上三組線圈 U/V/W,三組線圈在圓周上每隔 120°(電氣角)分布;[§11](encoders.md) 的三顆霍爾開關也嵌在定子上;§12 功率橋的三條輸出線,接的就是定子的 U/V/W。轉子則是轉動的部分(= 輪框):一圈永久磁鐵,N/S 交替排列,[§11.1](encoders.md) 的「極對數」就是數它。§17.3 有一張 BLDC 剖面圖,畫的是**內轉子**型——線圈在定子、磁鐵在轉子的分工一模一樣,只是內外對調;輪轂馬達是**外轉子**,轉子在外圈,那一圈就是輪框。
