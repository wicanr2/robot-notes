...
1. **電平轉換免費送**:輸出高電平由「上拉接到哪」決定,跟晶片自己的供電無關。5V 供電的霍爾開關,上拉到 3.3V,輸出擺幅就是 0–3.3V——直接安全接 STM32,不用電平轉換晶片。這正是 [§11.3](encoders.md) 接線圖把上拉接 3.3V 的原因。
...
> 延伸:[§6](communication-buses.md) 的 CAN 匯流排「顯性/隱性」電平機制,本質上就是同一個 wired-AND 思想的差分版——任何節點都能把匯流排打成顯性(0),沒人發送時回到隱性(1),這是 CAN 硬體仲裁(ID 小者勝出)的物理基礎。
...
想用 NMOS 放上面推高?那就回到 [§12.4](motors-and-foc.md) 的坎 2:Gate 要比 Source 高,需要自舉電路——為一支邏輯腳位加 bootstrap 完全不划算。所以「一顆接地的 NMOS 拉低」是**最小、最便宜、驅動最簡單、導通電阻最低**的開關,開漏設計剛好只需要這一顆。
