...
<p align="center"><img src="../../img/max-entropy.svg" width="580" alt="同樣平均與變異數下,高斯最攤平=假設最少(最大熵)"></p>
...
### 補一條:為什麼「配得好不好」總是變成平方和

這一條不在四條性質裡,但導航那幾篇一直在用它——scan matching 為什麼最小化殘差**平方**和、pose graph 為什麼**按協方差加權**攤回誤差,答案都在這裡,而且只要取一次對數。

**設定**:有一堆量測,每一筆的誤差(殘差)`rᵢ` 假設是獨立的零均值高斯,標準差 `σᵢ`。問「哪一組參數最可能產生我看到的這些量測」——這叫**最大似然**。

把每筆的機率密度乘起來:

```
L = Π  (1/√(2πσᵢ²)) · exp( −rᵢ² / (2σᵢ²) )
```

連乘不好處理,取對數變成連加(對數是遞增函數,所以讓 `L` 最大跟讓 `log L` 最大是同一件事):

```
log L = 常數 − ½ · Σ ( rᵢ² / σᵢ² )
```

常數與參數無關,前面那個負號把最大變最小。於是:
...
1. **為什麼是平方和,不是絕對值和。** 因為高斯的指數項裡就是平方——平方和不是誰選的,是「誤差是高斯」這個假設的直接後果。誤差若假設成拉普拉斯分布,掉出來的就會是絕對值和。
2. **`1/σᵢ²` 就是那個「信心權重」。** 量得越準(`σ` 越小),這一筆的權重越大、越不容許被改動。多維的情況把 `1/σ²` 換成**資訊矩陣** `Σ⁻¹`,殘差平方變成 `rᵀ Σ⁻¹ r`(這個量叫 Mahalanobis 距離——用協方差當尺規量出來的「差幾個標準差」)。

> 這就是[SLAM 建圖](../10-core/30-navigation/slam-mapping.md#213-迴圈閉合-loop-closureslam-真正的難點)裡「按信心攤回」那句話的全部內容:loop closure 與 odometry 兩個約束打架時,誰的協方差小誰說話大聲,而這個規則不是工程慣例,是把高斯取一次 log 就有的。
