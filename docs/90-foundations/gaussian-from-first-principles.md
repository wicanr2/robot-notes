...
核心式子裡的關鍵是 $e^{-(\text{距離})^2}$:離中心越遠,機率以「距離平方」的速度急速但平滑地衰減。`σ` 控制衰減多快(鐘多胖),前面的 $\frac{1}{\sqrt{2\pi}\sigma}$ 只是讓整條曲線下面積=1(機率總和為 1)的歸一化係數。
...
**P1 — 只需兩個參數。** 一個 $d$ 維高斯完全由均值向量 $\mu$ 與協方差矩陣 $\Sigma$ 決定:

$$ \mathcal{N}(x;\mu,\Sigma)=\frac{1}{(2\pi)^{d/2}\,|\Sigma|^{1/2}}\exp\!\left(-\tfrac{1}{2}(x-\mu)^{\top}\Sigma^{-1}(x-\mu)\right) $$

參數量固定:$\mu$ 有 $d$ 個、$\Sigma$ 有 $d(d{+}1)/2$ 個,跟看過多少資料無關。

**P2 — 仿射變換後仍是高斯。** 若 $x\sim\mathcal{N}(\mu,\Sigma)$,對任意矩陣 $A$、向量 $b$:

$$ y = Ax + b \;\Longrightarrow\; y\sim\mathcal{N}\!\left(A\mu+b,\;A\Sigma A^{\top}\right) $$

新的 $(\mu,\Sigma)$ 直接套公式算出,不必重做積分。取部分分量(邊際化)就是拿一個挑選矩陣當 $A$,所以邊際分布也照這條走。

**P3 — 兩高斯相乘 → 高斯(資訊融合的根)。** 兩個高斯密度相乘、正規化後仍是高斯。一維:$\mathcal{N}(\mu_1,\sigma_1^2)\cdot\mathcal{N}(\mu_2,\sigma_2^2)\propto\mathcal{N}(\mu,\sigma^2)$,且

$$ \frac{1}{\sigma^{2}}=\frac{1}{\sigma_1^{2}}+\frac{1}{\sigma_2^{2}},\qquad \frac{\mu}{\sigma^{2}}=\frac{\mu_1}{\sigma_1^{2}}+\frac{\mu_2}{\sigma_2^{2}} $$

用**精度**(變異數的倒數)相加,結果更尖、更確定——這正是卡爾曼更新。多維就是資訊矩陣相加:$\Sigma^{-1}=\Sigma_1^{-1}+\Sigma_2^{-1}$、$\Sigma^{-1}\mu=\Sigma_1^{-1}\mu_1+\Sigma_2^{-1}\mu_2$。
...
- **可分離**:2D 高斯卷積 = 先做一次水平 1D 高斯,再做一次垂直 1D 高斯(因為 $e^{-(x^2+y^2)}=e^{-x^2}e^{-y^2}$),運算量從 O(n²) 降到 O(n),快很多。
