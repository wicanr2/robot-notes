...
(`Σ_ε` 是噪聲的**共變異數矩陣**,決定每個控制軸上撒多開。本節後面出現的 `Σ` 若帶上下標範圍就是求和符號,只有這個帶 `ε` 下標的是矩陣。)

這定義了一個**基礎分布** `p`——以當前控制序列為均值的高斯。把整條軌跡的代價寫成 `S(τ)`,`τ` 代表一整條軌跡;注意 `S` 可以是任何東西,只要算得出一個數字。
...
現在要找一個新的分布 `q`,它要同時滿足兩個彼此拉扯的要求:**期望代價要低**,但**不能離 `p` 太遠**(離太遠就等於放棄了「從當前控制序列附近搜尋」這件事,而且取樣會失效)。
...
- **`E_q[·]`**:在分布 `q` 底下取**期望值**,也就是「以 `q` 為權重的加權平均」。下標換成 `p` 或 `q*` 就是換一個分布來加權,同一個運算子。
- **`D_KL(q‖p)`**:**KL 散度**,衡量兩個分布差多少。它的定義就是一個期望值:

  $$ D_{\mathrm{KL}}(q\,\|\,p) \;=\; \mathbb{E}_q\!\left[\log \frac{q}{p}\right] $$

  性質是**恆非負,而且只有 `q = p` 時為零**。整段推導只會用到這一條性質,以及上面這條定義。
- **`J[q]`** 吃進去的不是一個數字或向量,而是**一整個分布**。這種「函數的函數」叫**泛函(functional)**,所以下面做的是「在所有可能的機率分布裡找最好的那一個」——聽起來像無窮維,但它有閉式解。
...
`Z` 是**歸一化常數**,角色和後面 softmax 的分母完全一樣——只是它是一個對所有軌跡的積分,算不出來。有了它,`q*` 才是個合法的機率分布(非負,積分為 1)。
...
**第二步**,注意 `q*` 的定義兩邊乘上 `Z` 就是 `Z\,q^* = e^{-S/\lambda} p`——分母那一整組正好等於它,直接**代換**進去(這不是通分,是拿一個已知的等式做替換):
...
>
> 兩個技術前提在實務上通常成立,但值得寫明:`q` 必須對 `p` **絕對連續**(否則 `D_KL(q‖p)` 是無限大,`J[q]` 本身沒有意義),而且 `Z = E_p[e^{−S/λ}]` 必須**有限**(否則 `q*` 不是合法分布)。
...
用 **`K` 條取樣軌跡**(Nav2 預設 `K = 1000`)做近似。關鍵在這裡:分子分母各自的 `1/K` 抵消,而那個算不出來的 `Z` **也被分母的求和取代掉了**:

$$ \mathbb{E}_{q^*}[f] \;\approx\; \sum_{k=1}^{K} w_k\, f(\tau_k), \qquad \boxed{\;w_k = \frac{e^{-S_k/\lambda}}{\sum_{j=1}^{K} e^{-S_j/\lambda}}\;} $$

**這就是 softmax**(把一組數字轉成一組加起來等於 1 的權重),而丟進去的那個數字 `−S_k/λ` 叫 **logit**。原本 `Z` 要對所有可能的軌跡積分,現在只要對手上這 `K` 條求和——**這就是重要性取樣買到的東西**。
...
| **NLP-MPC** | 至少要**可微**(允許非凸) | 梯度式數值最佳化 | 求解時間不定,非凸會卡局部最優 |
...
算一下規模:每個控制週期要前向模擬 `1000 × 56 = 56,000` 個位姿,再對每個位姿跑一遍所有 critic。這是 survey 的 Table II 裡 MPPI 只有 **125 Hz** 而 RPP 有 **>4000 Hz** 的**主要來源之一**——但不是唯一原因:兩者的運算性質根本不同,RPP 是純幾何閉式解、完全不模擬任何未來狀態,所以不能拿 56,000 去反推單一位姿的成本。
...
- **「不需要可微」不等於「沒有代價」。** §13.6 保證的是最優分布存在且有閉式形式,**不保證用有限樣本估得準**。代價的動態範圍太大時,`e^{−S/λ}` 會高度集中在極少數樣本上——名目上撒了一千條,實際有效的可能只剩個位數(有效樣本數塌陷),此時輸出就是那幾條的抖動。這與上一條的多峰問題是同一個病的兩種表現。
- **加權平均本身可能產生不可行解。** 每一條取樣軌跡都個別避開了障礙物,不代表它們的加權平均也避得開——當可行域是非凸的(繞障礙物的左右兩側各是一塊),平均值可能正好落在中間那塊不可行的區域。這比「硬約束是事後懲罰」更進一步:問題出在**平均這個操作**,不在單條軌跡的評分。
- **維度與時域的詛咒。** 控制自由度乘上時域步數變大時,固定條數的隨機取樣對搜尋空間的覆蓋率會迅速惡化。`K × T` 那條講的是算力,這條講的是**即使算力夠,撒得也不夠密**。
