...
| 斜坡目標(等速變化) | 2 | 貨叉等速上升時要跟得住,單一 I 項會留固定落後量 |
| 週期性擾動(頻率 ω 已知) | 對應 `1/(s² + ω²)` 的共振項 | 輪子偏心造成的週期性速度漣漪(重複控制) |

第二列在實務上會咬人:貨叉用位置環 + PID 追一條等速上升的軌跡,**穩態會留下一個固定高度落差**——因為斜坡輸入需要兩個積分器,PID 只給一個。解法不是把 `K_i` 拉大(那只會震盪),是**把速度前饋加上去**,讓回授只需要處理常值等級的殘差。
...
**病一:放大高頻噪聲。** 對訊號 `n·sin(ωt)` 微分得 `nω·cos(ωt)`——振幅被乘上 `ω`。編碼器量測裡 100 Hz 的抖動,微分後被放大 100 倍(相對於 1 Hz 的真實訊號)。實務上一律加低通,用**濾波微分**:
...
中括號裡對 `u` 是個**二次函數**,而且因為 `R > 0`,它是嚴格凸的——有唯一最小值,對 `u` 求導設為零就找得到:
...
用 `P` 對稱把 `2xᵀPAx` 寫成 `xᵀ(PA + AᵀP)x`,合併同類項:
...
用 `K = R⁻¹BᵀP`(於是 `BᵀP = RK`、`PB = KᵀR`)配上 Riccati 方程 `AᵀP + PA = PBR⁻¹BᵀP − Q`,把中括號逐項換掉:
...
這是很強的性質——馬達實際輸出比預期大一倍、或小到一半,系統照樣穩。它來自 LQR 的迴路傳遞函數滿足一個叫 **Kalman 不等式**(回歸差式)的頻域條件。
...
<p align="center"><img src="../../img/ctl-forklift-tricycle-kinematics.svg" width="860" alt="舵輪叉車的瞬時旋轉中心幾何:轉向輪垂線與固定軸垂線交於 ICR,由直角三角形 tan δ = L/R 導出 ω = v·tanδ/L;右側標出前輪轉向與後輪轉向兩種構型的 ICR 位置差異"></p>
...
這件事在控制上的代價是硬的:**右半平面零點限制了閉迴路能達到的頻寬**。增益拉高到某個程度以上,系統必定不穩——這不是調參技巧的問題,是系統本身的性質,任何控制器都繞不過去。粗略的界線是頻寬不能超過 RHP 零點的量級,也就是 `v/L`:**車速越快,可用頻寬越低**。叉車越開越快就越難精確跟線,原因在此。
...
最後兩列是叉車特有、而且是安全等級的:平衡重式叉車的穩定性靠配重與載重的力矩平衡,**貨叉舉高載重時整台車的穩定三角形會急遽縮小**。這是機構與安全規範的領域,不是控制器能補救的——控制器該做的是**不要走進那個區域**(限速、限轉彎半徑),而不是走進去之後再想辦法穩住。
...
**增益排程(gain scheduling)** 的做法:選幾個排程變數(叉車上最自然的是 `v` 與載重 `m`),在網格上的每個工作點離線解一次 Riccati,把 `K` 存成查表;線上量測當前的 `v` 與 `m`(載重可由液壓壓力感測器或稱重推得),查表 + 內插。
...
2. **終端代價 `x_Nᵀ P x_N` 裡的 `P`,通常就用 LQR 的 Riccati 解。** 它代表「有限時域結束之後,剩下的無限尾巴要付多少」。用 LQR 的 `P` 當終端代價,是保證 MPC 穩定性的標準做法。**LQR 沒有被取代,它變成 MPC 的零件。**
3. **代價是線上算力。** 每個週期要解一個二次規劃(QP)。叉車的路徑追蹤問題規模小(狀態 4–5 維、時域 10–20 步),現代 QP 求解器在工控機上跑 20–50 Hz 沒有問題;但這已經比 `u = −Kx` 的四次乘加貴了好幾個數量級,而且**求解時間不是定值**——即時系統要為最壞情況預留,或設迭代上限並備好退路。
...
# --- 離散時間(實機跑這個)---
Ad = np.eye(2) + A * T                        # 一階近似;精確版用 scipy.linalg.expm
Bd = B * T
...
- Safonov, M. G., Athans, M., "Gain and phase margin for multiloop LQG regulators," *IEEE Transactions on Automatic Control*, vol. AC-22, pp. 173–179, 1977. — §6.3 增益與相位裕度結果的多變量推廣。
