...
> ⚠ **這是舊版單檔整理,已被取代,不再更新。**本檔已整理進 [`docs/00-overview/system-architecture.md`](../00-overview/system-architecture.md)。本檔保留僅供歷史對照,內容可能與現行主題檔不一致——**請以主題檔為準**。


> 目標讀者:已知「上位機 (ROS2) + 下位機 (STM32 馬達控制)」分層概念,想理解完整架構、硬體選型與軟體組成的工程人員。
...
送餐機器人本質上是一台「室內 AMR + 餐廳應用層」,典型分成三層:

```
┌─────────────────────────────────────────────┐
│  應用層 (App Layer)                          │
│  點餐系統整合 / 桌位管理 / 任務調度 / UI / 語音 │
├─────────────────────────────────────────────┤
│  上位機 (High-level / ROS2)                  │
│  SLAM 建圖 / 定位 / 路徑規劃 / 避障 / 感測融合  │
│  運算平台:Jetson / x86 / RK3588             │
├──────────────── UART / CAN ─────────────────┤
│  下位機 (Low-level / MCU)                    │
│  馬達閉迴路控制 / odometry / IMU / 電源管理    │
│  安全防撞 / 急停 / 燈效 / 托盤感測             │
│  運算平台:STM32 (常見 F4 系列)               │
└─────────────────────────────────────────────┘
```
