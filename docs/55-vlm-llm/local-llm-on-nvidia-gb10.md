...
- 硬體規律:**精度減半,吞吐常常翻倍**。因為位元少一半,同樣的電路面積/頻寬單位時間能塞進去算的運算數就多一倍(這是近似規律,實際倍率看晶片設計)。
...
2. **有沒有含 sparsity(稀疏)?** **sparsity** = 假設權重裡有很多 0(可跳過不算),廠商常報「含 2:4 結構化稀疏」的理論峰值,實務上不一定吃得到。含 sparsity 的數字通常是不含的 **2 倍**。
...
換算到大家熟悉的 FP16 來比:FP4 → FP16 約再差一個量級(精度高 4 倍,吞吐約 1/4），**第三方分析把 GB10 換算成約 250 TFLOPS(FP16 dense)等級**(LMSYS / IntuitionLabs 的換算,非官方數字,見文末)。重點不是記住換算後的精確值,而是養成習慣:**任何「N PetaFLOP」都先問精度與 sparsity,再決定能不能跟另一個數字比**。

### 1.5 FLOPS 跟訓練 / 推論的關係

- **訓練**:用「總共要做幾次運算」(FLOPs 數量,大寫的反面)衡量成本——例如「訓練 GPT 級模型要 ~10²⁴ FLOPs」。算力(FLOPS 速率)越高,訓練越快。**訓練普遍需要較高精度**(FP16/BF16,甚至部分 FP32)以維持數值穩定,所以低精度的漂亮 FLOPS 數字對訓練幫助有限。
...
回憶上一篇 §5:LLM 推論是 autoregressive 的,**每生成一個 token,都要把模型的權重讀出來算一遍**。生成階段(decode)一次只處理一個 token,這是個「矩陣 × 向量」運算——
...
```
單序列每秒能生幾個 token  ≈  記憶體頻寬 ÷ 每個 token 要讀的權重 bytes
```

舉例(粗估,看數量級):一個 70B 模型用 FP16 存,權重約 140 GB。要生 1 個 token,理論上得把這 140 GB 讀一遍。記憶體頻寬若是 273 GB/s(GB10 的數字,見 §3),理論上限約 273 ÷ 140 ≈ **每秒 2 個 token**(實務還更低)。**換成資料中心級 HBM(數 TB/s),同一個模型就能快上十倍**。看到了嗎——決定速度的是**頻寬**,不是 FLOPS。
...
NVIDIA 為「桌上型本地 AI 開發」推出的產品。命名沿革容易混,先理清:
...
量化不是免費的:bit 越少,能表示的數值越粗,**模型精度會下降**。所以重點是「**用最少的 bit,把精度損失壓到可接受**」。NVIDIA 在 Blackwell 推出 **NVFP4**(一種 4-bit 浮點格式,每值為 FP4 E2M1:1 個符號 + 2 個指數 + 1 個尾數位),配合校準技術,讓大模型量化到 4-bit 仍能保住接近原本的準確度(官方稱可達 ~99% 準確度回復)。
...
跑 LLM 的瓶頸是**記憶體**(容量決定裝不裝得下、頻寬決定跑多快),不是單純算力。GB10 / DGX Spark 的設計就是順著這條原理:用 **128 GB 統一記憶體**換到「桌上就能塞下大模型」,代價是 LPDDR5x 的頻寬不如資料中心 HBM;再靠 **FP4 量化**同時壓低容量佔用與每 token 搬運量。它是「本地、離線、隱私」場景的好開發/部署平台,但不是訓練怪獸、也不取代機器人那層跑得快、可驗證的傳統控制。讀任何「N PetaFLOP」規格,記得先問**精度**與 **sparsity**(§1)。
...
- FP4→FP16 的**等效 FLOPS 換算值**(~250 TFLOPS 為第三方分析,非官方)。
