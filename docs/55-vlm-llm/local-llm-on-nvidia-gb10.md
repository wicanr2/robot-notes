...
> **把前面幾個數字湊起來算一次,才知道「跑得動」是什麼意思。** 200B 參數用 FP4 存,每個參數半個 byte → 權重約 **100 GB**(剛好塞得進 128 GB)。生一個 token 要把權重讀一遍,所以 `273 GB/s ÷ 100 GB/token ≈ 2.7 token/s`。
>
> **這就是「單機可跑 ~200B」的實際體感:塞得下,但慢。** 想要互動式的速度(10 token/s 以上),同樣的頻寬下模型得壓到 ~27 GB 以內——大概是 70B 用 FP4,或 30B 用 FP8。**容量決定跑不跑得動,頻寬決定跑得多快,兩者是不同的天花板。**
