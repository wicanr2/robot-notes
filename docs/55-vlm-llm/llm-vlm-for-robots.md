...
> 名詞:**token(詞元)** = 模型處理文字的最小單位,可能是一個字、半個英文單字、或一個標點。不是「一個字 = 一個 token」,中文常一字一 token 上下,英文常一個詞被拆成幾塊。(為什麼中英文待遇不同?切法是從訓練語料的統計學出來的:中文字本身就帶不少語意、組合多,單字成 token 較划算;英文字母本身沒語意、靠拼,於是常見字根/字尾被切成共用片段。)
...
同一個字,意思由**周圍哪些詞**決定——「開」把「會」拉向「會議」,「說語言」把「會」拉向「能夠」。模型若想正確處理一個 token,必須能「回頭看」整句話裡**跟它有關的其他 token**,而且自動判斷哪些相關、哪些無關。這就是自注意力要解的根本問題:**讓每個 token 動態地、按相關程度去「參考」序列裡其他 token 的資訊**。
...
回到上面「我下午有個會要開」:「會」這個 token 的 query,會和「開」的 key 高度相似 → 給它高權重 → 於是「會」的表示被「開」拉向「會議」語意(而非「能夠」)。**整句話的詞,就這樣互相參考、彼此消歧**。下面這張圖把這個配對過程畫出來:

<p align="center"><img src="../../img/self-attention-qkv.svg" width="720" alt="自注意力 query/key/value:目標 token「會」發出 query,去和句中每個 token 的 key 比對相似度(「開」最高、「下午」次之),相似度當權重把各 token 的 value 加權平均,匯總成「會」的新表示,於是被「開」拉向會議語意"></p>

多疊幾層,模型就能表示越來越複雜的關係(片語 → 句子 → 段落邏輯)。
...
務實的結論是**分層**:讓 VLM/VLA 處理「高層理解與決策」(聽懂指令、認出物件、決定去哪、做什麼),把「**安全攸關的底層**」——避障、急停、運動控制——留給傳統、可驗證、跑得快的模組(就是本 repo [導航](../10-core/30-navigation/path-planning.md) 與 [下位機控制](../10-core/20-firmware/low-level-control.md) 講的那一套)。大模型當「大腦皮層」,傳統控制當「脊髓反射」。
...
| 用 LLM 解析自然語言指令、轉成任務 | **大致可用**:配合明確的工具/API 框架(function calling,即讓模型輸出結構化指令去呼叫外部工具/API)效果好,但需防幻覺 |
