...
**為什麼要 inflation?**(第一性原理)因為**機器人有體積,不是一個點**。若規劃時把車當點,算出的路會緊貼牆面,實際開過去就撞上。膨脹層把障礙「長胖」:障礙格本身是 **lethal(致命)**;在**內切半徑**內直接給致命代價;再往外用**指數衰減**鋪漸層(離障礙越遠代價越低),鼓勵車走中間、保持安全距離。
...
關鍵參數:`inflation_radius`(膨脹鋪多遠)、`cost_scaling_factor`(代價隨距離衰減多快)。判準:車的 footprint(輪廓)永遠不可碰到致命格。
...
| 控制器 | 一句話 | 取捨 |
|---|---|---|
| **RPP**(Regulated Pure Pursuit,調節型純追蹤) | 幾何純追蹤 + 急轉/近障自動減速 | 簡單省算力、忠實跟線,**不做動態避障** |
