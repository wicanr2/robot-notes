...
算一次給個具體感覺。部署時登錄地標 M 固定在世界座標 $(10.0,\ 5.0)$、朝向已知;相機當下量到三件事:M 在正前方**偏右 $15°$**、距離 **$2.0\ \text{m}$**、而且我是**斜看它 $30°$**。

要解的是三個未知數 $(x, y, \theta)$,所以需要三個量測,順序不能反:

1. **先解朝向。** 「我斜看它 $30°$」配上地標已知的朝向,直接定出我的 $\theta$——這裡得到 $\theta = 27°$。位置還完全不知道,但朝向已經確定。
2. **把機體系的方位角轉到世界系。** 偏右 $15°$ 是相對我的車頭量的,換算成世界方位角就是 $\theta - 15° = 12°$。
3. **從地標倒推位置。** 沿那個方位角退 $2.0\ \text{m}$:

$$(x,\ y) = (10.0,\ 5.0) - 2.0 \cdot (\cos 12°,\ \sin 12°) = (8.0,\ 4.6)$$
