...
用式子寫:把 scan 第 `i` 個點在車身座標記為 `pᵢ`,在初猜位姿上疊加微調 `Δ = (Δx, Δy, Δθ)` 後的座標變換記為 `T_Δ`(先旋轉 Δθ、再平移 (Δx, Δy)),要找的是

$$ \Delta^{\star}=\arg\min_{\Delta}\;\sum_{i=1}^{N}\,d\!\big(\mathcal{M},\,T_\Delta(p_i)\big)^{2} $$

其中 `M` 是已建好的地圖,`d(M, ·)` 是「把這個 scan 點擺到世界座標後,它到地圖上最近障礙的距離」(殘差)。`N` 個點的殘差平方和就是代價函數,讓它最小的 `Δ*` 就是這一幀的位姿修正量。
