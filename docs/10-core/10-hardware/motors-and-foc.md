...
> **沒碰過馬達的話,建議讀法**:**§17(有刷 vs 無刷:馬達到底是什麼)→ §16(定子與轉子)→ §2(FOC)→ §12(功率橋)→ §9(看懂型錄)**。
> 本檔編號沿用原始《送餐機器人基礎原理補充》方便與舊文件對照,**編號順序不是閱讀順序**——§2 的 FOC 會用到 §16/§17 才定義的詞。
>
> 章節編號沿用原始文件(故本檔編號不連續,如 §1→§5→§10,非缺漏)。
...
要看懂這些詞,先知道「讓 BLDC 轉起來」的完整硬體鏈長什麼樣:MCU 算出 PWM,交給閘極驅動器放大成能推動 MOSFET 的訊號;功率橋(6 顆 MOSFET)真正開關大電流、把電池 24V 切成三相波形;最後送進三相線圈,馬達轉動。完整圖解與「大腦/神經肌腱/肌肉」的角色對照見 §12.5。
...
### 12.2.5 PWM:只能全開全關的東西,怎麼調出連續可變的電壓

上一段結尾那句「只工作在全開/全關」會立刻引出一個問題:**那平滑可調的電壓是哪來的?** 答案不在元件裡,在時間軸上。

<p align="center"><img src="../../../img/motor-pwm-duty-cycle.svg" width="820" alt="PWM:方波的占空比決定平均電壓,線圈電感把方波電壓磨成平滑電流"></p>

**PWM(Pulse Width Modulation,脈寬調變)** 的想法只有一句:開關開得**夠快**時,負載感受到的不是瞬間值,而是**平均值**。

- **占空比(duty cycle)** = 一個週期裡「開」佔的時間比例。24V 電源、占空比 50%,平均電壓就是 12V;占空比 25% 就是 6V。**電壓從離散變連續,靠的是時間比例而不是元件。**
- 「夠快」要多快:對馬達,10–20 kHz。這個頻率遠高於馬達的機械與電氣反應速度,所以馬達「來不及」跟著方波抖。順帶一提,選在 20 kHz 附近還有個實務理由——避開人耳聽得見的範圍,不然馬達會叫。
- 電壓還是方波,但**電流不是**。線圈本身是電感,電感的性質就是抗拒電流突變(`v = L·di/dt`),等於天生的低通濾波器。方波電壓進去,出來的是平滑的三角波電流,漣波大小由電感值與 PWM 頻率決定。

**所以不需要真的產生弦波電壓,只要讓每一相的平均電壓照弦波變化就行。** 這就是為什麼六顆只會開關的管子,能推出近似弦波的三相電流。

**SVPWM(Space Vector PWM,空間向量脈寬調變)** 是決定「這三個占空比該設多少」的演算法。FOC 算完之後得到的是一個電壓向量(大小 + 方向),SVPWM 的工作是把它換算成六顆管子各自的開關時間。名字裡的 space vector 就是指那個向量——它把三相電壓當成一個平面上的向量來湊,而不是三條獨立的弦波,所以能比單純三條弦波多榨出約 15% 的電壓利用率。

> 這一節解鎖了三個地方:[§2.2](#22-核心思想) 的第 ④ 步「反 Park + SVPWM → 三相 PWM 占空比」、下面 §12.3 的「六步換相 vs FOC」、以及[電源與安全](power-and-safety.md)裡的「封鎖 PWM」——那三處講的都是同一件事。
...
定子不動,固定在輪軸上:矽鋼片疊成的鐵芯,開槽,繞上三組線圈 U/V/W,三組線圈在圓周上每隔 120°(電氣角)分布;[§11](encoders.md) 的三顆霍爾開關也嵌在定子上;§12 功率橋的三條輸出線,接的就是定子的 U/V/W。轉子則是轉動的部分(= 輪框):一圈永久磁鐵,N/S 交替排列,**成對數就是[極對數](encoders.md#111-原理三顆霍爾開關--轉子磁鐵)**(常見 15 對),它決定了電氣角與機械角差幾倍。§17.3 有一張 BLDC 剖面圖,畫的是**內轉子**型——線圈在定子、磁鐵在轉子的分工一模一樣,只是內外對調;輪轂馬達是**外轉子**,轉子在外圈,那一圈就是輪框。
