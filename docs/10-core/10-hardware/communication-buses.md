...
STM32 腳位輸出的是 0/3.3V 單端數位訊號,匯流排上跑的是差分電平,中間**必須加一顆收發器 (transceiver)** 做轉換——這是接線的核心觀念,完整接線見下一節的圖。
...
- 收發器常用 TJA1051、SN65HVD230(3.3V 直接配 STM32);**120Ω 終端電阻只放匯流排最遠兩端**,中間節點不放。
  - **為什麼是 120Ω、為什麼只放兩端**:訊號在雙絞線上跑到底,如果末端沒有東西「吃掉」它,能量會**反射**回來疊在後續的訊號上,把波形弄糊。要不反射,末端接的電阻必須等於線本身的**特性阻抗**——雙絞線約 120Ω,這個數字是線的物理性質決定的,不是規範挑的。**只有兩端會反射**(中間節點兩側都有線繼續延伸),所以只在兩端各接一顆;多接一顆就等於把匯流排負載加重,訊號幅度反而被拉低。
  - **顯性 / 隱性在電位上是什麼**:CAN 用差動,`CANH − CANL`。沒有人主動驅動時,兩線被拉到差不多電位,差值 ≈ 0V,匯流排讀作 **1(隱性,recessive)**——「隱性」的意思就是「不出力,誰都可以蓋過我」。有節點要送 0 時,它主動把 CANH 拉高、CANL 拉低,差值約 2V,讀作 **0(顯性,dominant)**。**主動驅動一定贏過不驅動**,這就是「0 蓋過 1」的物理來源,也是仲裁不必協調就能收斂的原因。
