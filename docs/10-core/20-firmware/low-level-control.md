...
1. **差速運動學正逆解**([§1.1](../../20-forms/wheeled-amr/chassis-and-drivetrain.md) 的兩組公式)——下位機核心邏輯就是逆解,odometry 就是正解的積分。
...
3. **Odometry 積分**(航位推算):
   ...
   **這幾條也不用背,是推出來的。** 第一性原理:第 1 行 `Δs`、`Δθ` 就是 [§1.1](../../20-forms/wheeled-amr/chassis-and-drivetrain.md) 的差速正解(把輪速換成這一步的弧長);第 2、3 行則是把「車**朝著車頭方向**前進了 Δs」這件事,投影到世界座標的 x、y 軸——一個朝向 θ 的向量,水平分量是 `Δs·cosθ`、垂直分量是 `Δs·sinθ`,cos/sin 就是這麼來的,不是公式背下來的。

<p align="center"><img src="../../../img/odometry-integration.svg" width="540" alt="odometry 一步積分:車頭方向位移 Δs 投影到世界 x/y 軸,用中點朝向 θ+Δθ/2"></p>

   唯一的細節是投影該用哪個朝向:這一步車頭從 θ 轉到 θ+Δθ,**用中點 θ+Δθ/2** 投影比用起點 θ 準——這是數值積分的中點法,把單步誤差**從一階降到二階**(不是消到零;直行 Δθ=0 時兩者相同)。

   > 嚴謹一點:等曲率圓弧的精確解還帶一個弧長修正因子 `sinc(Δθ/2) = sin(Δθ/2)/(Δθ/2)`,中點公式等於把它近似為 1。控制週期短、Δθ 很小時 `sinc≈1`,中點法已足夠——這正是高頻(100Hz)取樣下可以省略它的原因。
   ...
   | b | 輪距:左右兩驅動輪與地面接觸點的中心距 | m | 量測+標定([§1.1](../../20-forms/wheeled-amr/chassis-and-drivetrain.md) 的同一個 b) |
...
<p align="center"><img src="../../../img/loc-kinematics-pid-flow.svg" width="600" alt="運動學逆解(開環算目標)接PID(閉環追目標),encoder實測只回授進PID,不回授運動學"></p>
