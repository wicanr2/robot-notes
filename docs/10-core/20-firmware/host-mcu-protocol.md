...
- **LEN 要做合理性 gate(安全):** 接收端不該無條件相信 LEN。**每個 CMD 的合法 payload 長度應在命令表寫死**,收到的 LEN 與該 CMD 預期不符(或超過上界)就直接當假幀頭丟棄、re-sync。1-byte LEN 上限 255 風險有限;但**若擴成 2-byte LEN,一個雜訊/惡意的超大 LEN 會讓接收端癡等超長 payload、吃光緩衝**,構成 buffer 耗盡 / DoS 面,上界檢查就成了必要防線。
...
> **兩個名詞先翻譯**。**GF(2)** 是「只有 0 和 1 兩個元素的算術系統」:加法與減法都變成 XOR(1+1=0),沒有進位。CRC 的除法就在這個系統裡做,所以硬體用幾個 XOR 閘與移位暫存器就跑得動,不必真的做除法。**Hamming distance** 是「兩個合法訊息之間至少差幾個位元」——差越多,就越不可能因為隨機翻幾個位元而剛好從一個合法訊息變成另一個。
>
> **為什麼 16 位 CRC 能「保證」抓出 ≤16 位元的突發錯誤**:一段連續的位元錯誤可以寫成一個次數小於 16 的多項式。而 CRC 的除數是一個**16 次**多項式——次數比它低的東西除不盡,餘數必然不為零。所以這個保證不是統計上的,是代數上的:**低次的東西不可能被高次的整除**。

一個 16 位的 CRC 可以**保證抓出所有長度 ≤16 位元的突發錯誤**,對更長的突發錯誤則以約 2⁻¹⁶ 的漏抓率偵測(那個 2⁻¹⁶ 是「長突發」這類的殘餘漏檢量級;對「少數隨機位元錯」的保證另取決於該多項式在特定資料長度下的 Hamming distance)。這正是它壓過簡單 checksum 的根本原因——不是「算得比較複雜所以比較好」,而是它的代數結構天生對應到「線路最常見的錯誤型態」。
...
| CMD | 名稱 | payload(概念) | 頻率 | 說明 |
|---|---|---|---|---|
| `0x81` | odometry 回報 | `int32 x` + `int32 y`(mm)+ `int32 θ`(mrad)+ `int16 v` + `int16 ω`(+ 選 `uint32 t_ms`) | 50–100Hz | 對應 [low-level-control](low-level-control.md) §4.1 odometry 積分結果 |
