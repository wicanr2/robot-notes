...
> Nav2 的規劃器全部是**搜尋式**而非取樣式(RRT 那一類)。survey 給的理由是:在移動機器人這種低維狀態空間裡,搜尋式通常更快、在啟發式可採納(admissible)時能給出真正的最優解,而且**執行時間比較可預測**——後者對「要頻繁重規劃」的系統特別重要。

### 2.1 NavFn:波前傳播出一個位能場,再滑下去

Nav2 最老的規劃器,從 ROS 1 導航堆疊繼承而來。它的做法和 A* 那種「維護 open list」的寫法長得不太一樣:
...
前面三個都把車當成一個可以往任意方向平移的點。**叉車與 Ackermann 車不行**——[§7 那條非完整約束](../../90-foundations/feedback-control-pid-lqr.md#72-這是一個非完整約束)決定了它不能側移,而且轉彎半徑有下限。
...
- **`h_obstacle`**:對障礙物地圖做動態規劃距離擴散,得到「**忽略運動學**、純粹繞開障礙」的最短距離。
...
信念不是一個高斯,是**一堆帶權重的粒子**,每個粒子是一個位姿假設 `(x, y, θ)`:

1. **預測**:車動了,依 odometry 增量用**運動模型**對每個粒子取樣新位姿(加噪聲)。
2. **更新**:來了一幀雷射,用**量測模型**算每個粒子的 likelihood,乘進權重。
...
- **`k`** 是粒子在 kd-tree 裡佔了幾個非空格子——也就是**粒子散得多開**。
- **`ε`**(參數 `kld_err`)是允許的 KL 散度誤差,**`z`**(參數 `kld_z`)是常態分位數。
- 那個立方括號是**卡方分布分位數的 Wilson–Hilferty 近似**。
