...
**roll / pitch / yaw 就是繞這三根軸的轉動**,順序跟 x / y / z 一一對應——軸向定了,這三個名字不必另外背:

<p align="center"><img src="../../../img/nav-roll-pitch-yaw.svg" width="760" alt="roll 繞 x 前後軸為左右側傾,pitch 繞 y 左右軸為前後俯仰,yaw 繞 z 垂直軸為水平轉向"></p>

| 名稱 | 繞哪根軸 | 直覺 | 對輪式車 | 對足式 |
|---|---|---|---|---|
| **roll**(側傾) | **x**(前後軸) | 車身左右歪 | 幾乎恆為 0 | 會變,要估 |
| **pitch**(俯仰) | **y**(左右軸) | 車頭上下點 | 幾乎恆為 0(上下坡才變) | 會變,要估 |
| **yaw**(偏航) | **z**(垂直軸) | 原地轉向 | **唯一會變的角度** | 會變,而且[估不準](../../20-forms/legged/legged-fundamentals.md#5-狀態估計沒有輪子可以數) |

最右邊那兩欄就是輪式與足式在狀態估計上的分水嶺:平面上的輪式車只需要 `(x, y, yaw)` 三個量,roll 與 pitch 當常數處理不會出大錯;足式的軀幹六個自由度全都在動,一個都不能省。
