...
| 與 odometry 的關係 | ①步直接消費 [§20](../10-hardware/encoders.md) 上報的 odometry;odometry 越準,粒子噪聲可調越小,定位越穩——又一次回扣 [§4.1](../20-firmware/low-level-control.md) |
...
實務組合:離峰建一張乾淨地圖([§21.4](slam-mapping.md))+ 調高 `z_rand`/`z_short` 容忍動態遮擋 + odometry 標定紮實 → 人群中定位依然穩。這三件事分別對應 [§21](slam-mapping.md)、本節、[§4.1](../20-firmware/low-level-control.md)——導航品質從來不是單一模組的事。
...
算一次給個具體感覺。部署時登錄地標 M 固定在世界座標 `(10.0, 5.0)`,正面朝向 **57°**(這個值是登錄地標時一起記下來的);相機當下量到三件事:M 在正前方**偏右 15°**、距離 **2.0 m**、而且我是**斜看它 30°**。

要解的是三個未知數 `(x, y, θ)`,所以需要三個量測,順序不能反:

1. **先解朝向。** 「我斜看它 30°」意思是我的視線與地標正面法線夾 30°。地標正面朝 57°,所以我站在 `57° − 30° = 27°` 那個方向上,也就是 `θ = 27°`。位置還完全不知道,但朝向已經確定。
2. **把機體系的方位角轉到世界系。** 偏右 15° 是相對我的車頭量的,換算成世界方位角就是 `θ − 15° = 12°`。
3. **從地標倒推位置。** 沿那個方位角退 2.0 m:

```
(x, y) = (10.0, 5.0) − 2.0 · (cos 12°, sin 12°) = (8.0, 4.6)
```

三個量測給定,位姿就唯一確定,沒有調整空間——這正是它「沒有多義性」的來源。少量一個(例如只有距離與方位角、沒有斜看角度),朝向就解不出來,得另外靠 odometry 或第二個地標補。
...
| 反光柱(LiDAR 版地標) | 高反射率柱子,LiDAR 強度值直接認出 | 工業 AGV 經典做法(對應 [§18.3](../10-hardware/power-and-safety.md) 的 ISO 3691-4 世界) |
...
設計建議:本專案以「LiDAR + AMCL 為主、AprilTag 為輔(回充對接 + 開機初始化 + 特徵貧乏區補強)」——跟頭部產品([§24](../10-hardware/sensors.md))的演進路線一致:標記從「主定位」退役後,正是在這些角落繼續服役。
