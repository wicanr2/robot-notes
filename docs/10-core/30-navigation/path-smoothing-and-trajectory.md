...
> **這條式子不是另一個定義,它就是 §1 那個 `κ = dθ/ds` 換成參數式的樣子。** 兩步:
>
> 1. **方向角**是切向量的角度,`θ(t) = atan2(y', x')`。對 `t` 微分(套 atan2 的微分)得到 `dθ/dt = (x'y'' − y'x'') / (x'² + y'²)`。
> 2. **弧長**沿曲線走的速率是 `ds/dt = |p'(t)| = √(x'² + y'²)`。
>
> 相除就是 `dθ/ds`——分母多出來的那個 `√(x'²+y'²)` 正是第 2 步貢獻的,所以指數才會是 `3/2` 而不是 `1`。**參數化怎麼選都不影響 κ**,因為它問的是「每走一段**弧長**轉多少」,跟你用多快的 `t` 掃過去無關。
...
Bézier 只有控制點,B-spline 多了第二組東西:**節點(knot)**。這是後面所有性質的地基,所以先講清楚它長什麼樣。

<p align="center"><img src="../../../img/traj-knot-vector.svg" width="860" alt="節點向量是參數軸上的分段點:單重節點處三次 B-spline 自動 C2 連續;把節點疊成 k 重就降為 C^(p−k),疊到 p 重變成尖角"></p>

一條 B-spline 不是一條式子畫到底,而是**好幾段拼起來**的。節點就是**參數軸上的分段點**——曲線在這些位置從一段換到下一段。把 `[0, 1]` 這條參數軸標上一組刻度,那組刻度就是**節點向量**。

兩個詞先釘住:

- **單重節點**:每個分段點各自佔一個位置。
- **節點重數 `k`**:把 `k` 個分段點**疊在同一個參數值上**。疊得越多,那個接縫就越「硬」——每疊一重就少一階連續性。

有了這個圖像,下面兩條性質就不必背了:接縫平不平滑取決於**疊了幾重**,而動一個控制點會影響幾段,取決於**它跨過幾個節點區間**。

### 5.1.1 Cox–de Boor 遞推

> **這條遞推式可以先跳過。** 它是基底函數的正式定義,實作時交給函式庫;真正要理解的是下一節那兩條性質。
...
**(a) 連續性內建。** `p` 次 B-spline 在**單重**內部節點處自動 `C^{p−1}` 連續;節點重數為 `k` 時降為 `C^{p−k}`(就是上面那張圖的兩排)。
...
**(b) 局部支撐。** `N_{i,p}(u)` 只在 `[u_i, u_{i+p+1})` 上非零——也就是說,一個控制點只在**它跨過的那幾個節點區間**裡有發言權。因此:
