...
- **不要把來源的「統計上偏向」升級成「幾何上的必然」**:四足步態那篇原本寫「duty factor < 0.5 必有飛行相,這是幾何必然不是經驗值」。查證資料講的是「統計上偏向」,我在改寫時把語氣加強了,而加強的那一部分沒有任何證據。實際門檻是 `1/n`(n = 獨立錯開的支撐組數),四腳各自錯開是 0.25,只有配對成兩組時才是 0.5;動物身上還有現成反例(amble:duty factor < 0.5 卻無騰空相)。**判斷法:寫「必然 / 一定 / 幾何上」之前,要能當場把證明寫出來;寫不出來就照來源的語氣寫。** 是學生審查員先提出質疑,專家審查員獨立確認並補上反例文獻的。
