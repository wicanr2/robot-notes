...
**支援的 STM32 系列(查證自官方 supported-boards 文件):** Renode 內附 STM32 F0、F4、F7、G0、H7、L0/L1/L5、WB/WL、WBA 等系列的平台描述。文件中明確列出的板子包含 STM32F4 Discovery、STM32F7 Discovery、ST Nucleo-64、以及 STM32F103「Blue Pill」等;近期也加入了 Cortex-M33 的 STM32WBA(BLE 無線 SoC)。周邊覆蓋 UART / SPI / I2C / GPIO / DMA / ADC / Timer / 看門狗等。
...
跑起來之後常用的:`pause`(暫停)、`machine Reset`(重置)、`cpu LogFunctionNames true`(印出呼叫到哪個函式做 trace,需 ELF 含符號表)、`logFile @path`(把 log 導到檔案)。

把上面那串指令存成一個 `.resc`(Renode script)檔,就能一鍵重建整個模擬環境——這也是 CI 與多人共享環境的基礎:

```
# firmware.resc —— 存成檔案後,一行啟動:renode firmware.resc
mach create
machine LoadPlatformDescription @platforms/cpus/stm32f4.repl  # 官方內附,或自寫
sysbus LoadELF @firmware.elf
showAnalyzer sysbus.uart2
start
```

> 安裝(對應 Docker first 原則):除了官方 release 包,可直接用官方 Docker image `antmicro/renode` 跑,不污染系統環境;CI 上常用 `renode-test` 跑 Robot Framework 測試。`.repl` 平台描述檔可從 repo 的 `platforms/cpus/` 取現成的改,最小骨架就是「一顆 cpu + 一段 memory + 一個 uart」。

### 用 GDB 像連真板一樣除錯

Renode 不只能跑,還能讓你用熟悉的 GDB 下中斷點、單步、看暫存器——真機除錯的直接替代,但不需要實體板與 ST-Link:
...
(gdb) target remote :3333      # 接上 Renode 的 GDB server
(gdb) break ControlTask_100Hz  # 在控制迴路下中斷點
(gdb) continue / step / info registers
```

也可接 VS Code 的 cortex-debug 擴充,把模擬器當成除錯目標,體驗跟連真板幾乎一樣。
