...
這套「多項式除法」(在 GF(2) 上、用 XOR 取代減法)的數學性質,讓 CRC 對突發錯誤特別強:一個 16 位的 CRC 可以**保證抓出所有長度 ≤16 位元的突發錯誤**,對更長的突發錯誤則以約 2⁻¹⁶ 的漏抓率偵測。(2⁻¹⁶ 是「長突發」這類的殘餘漏檢量級;對「少數隨機位元錯」的保證另取決於該多項式在特定資料長度下的 Hamming distance。)這正是它壓過簡單 checksum 的根本原因——不是「算得比較複雜所以比較好」,而是它的代數結構天生對應到「線路最常見的錯誤型態」。
