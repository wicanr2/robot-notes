...
1. **差速運動學正逆解**([§1.1](../10-hardware/chassis-and-drivetrain.md) 的兩組公式)——下位機核心邏輯就是逆解,odometry 就是正解的積分。
...
3. **Odometry 積分**(航位推算):
   ...
   **這幾條也不用背,是推出來的。** 第一性原理:第 1 行 `Δs`、`Δθ` 就是 [§1.1](../10-hardware/chassis-and-drivetrain.md) 的差速正解(把輪速換成這一步的弧長);第 2、3 行則是把「車**朝著車頭方向**前進了 Δs」這件事,投影到世界座標的 x、y 軸——一個朝向 θ 的向量,水平分量是 `Δs·cosθ`、垂直分量是 `Δs·sinθ`,cos/sin 就是這麼來的,不是公式背下來的。
...
<p align="center"><img src="../../img/odometry-integration.svg" width="540" alt="odometry 一步積分:車頭方向位移 Δs 投影到世界 x/y 軸,用中點朝向 θ+Δθ/2"></p>
   ...
   | b | 輪距:左右兩驅動輪與地面接觸點的中心距 | m | 量測+標定([§1.1](../10-hardware/chassis-and-drivetrain.md) 的同一個 b) |
